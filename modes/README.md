...
![에뮬레이터 화면](docs/screens.png)

1 흰 막대 · 2 엣지 패널 · 3 원형 메뉴 · 4 원형을 위로 돌림 · 5 왼쪽에 붙임 · 6 텅 빈 새 바탕화면 · 7 빈 곳 길게 → 편집 · 8 앱 추가·폴더 · 9 앱 고정 확인 창 · 10 기기 소유자 잠금
...
버튼은 화면 끝의 얇은 막대다(아이콘 없음). 색·길이·굵기·투명도, 왼쪽/오른쪽을 설정에서 고르고, 막대를
끌어 위아래로 옮기거나 가운데를 넘겨 반대쪽 끝으로 붙인다. 보이는 막대는 가늘어도 누르는 자리는 넉넉하다.

막대를 누르면 **엣지 패널처럼** 그 끝에서 키 큰 둥근 패널이 미끄러져 나오고, 모드들이 큰 타일과 이름으로
세로로 늘어선다(앱을 여는 모드는 그 앱의 진짜 아이콘, 많으면 굴림, 아래에 설정·편집, 지원하면 뒤를 흐리게).
설정의 "메뉴 모양"을 원형으로 바꾸면 Circle처럼 모드들이 반원을 그리며 알약 모양(바깥쪽 원 + 안쪽 이름)으로 튀어나온다(오버슈트
애니메이션, 차례로 조금씩 늦게). 한 번에 보일 개수(3~9)를 넘으면 원을 손가락으로 돌려서 본다. 버튼이 화면 위·아래 끝에 가까우면 반원을 돌려 잘리지 않게 하고, 가운데
✕나 바깥을 누르면 거품들이 버튼으로 빨려 들어가며 닫힌다.

## 새 바탕화면 편집 — 진짜 홈처럼

비밀 바탕화면은 **텅 빈 새 홈**으로 시작한다(기본 홈의 앱은 가져오지 않는다).

- 빈 곳을 길게 누르거나 ⋮ → 바탕화면 편집 → 편집 모드(칸이 점선으로 보인다).
- 빈 칸을 누르면 그 칸에 앱, 위의 **+ 앱**으로 여러 개를 한꺼번에.
- 앱을 길게 눌러 끌면 옮기기. 앱 위에 놓으면 **폴더**, 폴더 위에 놓으면 그 폴더에 넣기.
- 끄는 동안 위에 나타나는 🗑에 놓으면 삭제. 화면 가장자리에 머물면 옆 페이지로(없으면 새로 만든다).
- **+ 페이지**, 빈 페이지 삭제, 아래 **독** 5칸. 폴더를 누르면 열리고, 편집 중엔 이름을 바꾸고 앱을 뺄 수 있다.
- 배치는 저장되고, 한 페이지는 4×5칸.
...
| `SecretDesktopActivity.kt` · `ui/HomeScreen.kt` · `Desktop.kt` | 비밀 바탕화면 — 빈 새 홈 편집(칸·폴더·독·페이지), 생체 인증, 최근 앱·스크린샷 숨김 |
